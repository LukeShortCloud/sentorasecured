...
> /tmp/users.id; > /tmp/users.name; > /etc/sentora/trueuserdomains;
...
for x in `cat /tmp/users.name`;  
	...
	for z in `mysql -e 'use sentora_core; select vh_name_vc from x_vhosts where vh_acc_fk="'"$id"'";' | grep -v vh_name_vc`
		do echo "${x}${z}" >> /etc/sentora/trueuserdomains;
	done; 
done;

#Cleanup
rm -rf /tmp/users.*; chown root: /etc/sentora/trueuserdomains; chmod 660 /etc/sentora/trueuserdomains
